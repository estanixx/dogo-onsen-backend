...
@ItemRouter.get("/", response_model=list[Item])
async def list_items(session: AsyncSession = Depends(get_session)):
    return await ItemService.list_items(session)


@ItemRouter.post("/", response_model=Item, status_code=status.HTTP_201_CREATED)
...
@ItemRouter.get("/{item_id}", response_model=Item)
...
@ItemRouter.put("/{item_id}", response_model=Item)
